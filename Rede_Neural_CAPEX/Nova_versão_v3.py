...
def avaliar_qualidade_informacao(texto):
    texto_limpo = texto.strip()
    palavras = re.findall(r"\b\w+\b", texto_limpo.lower())

    qtd_palavras = len(palavras)

    tem_modelo_padrao = all(campo in texto_limpo.lower() for campo in [
        "objetivo",
        "equipamento",
        "tipo de intervenção",
    ...
    verbos_acao = [
        "substituir", "trocar", "instalar", "executar", "reparar",
        "automatizar", "integrar", "desenvolver", "adequar",
        "modernizar", "reformar", "realizar", "implantar"
    ]

    termos_tecnicos = [
        "sistema", "equipamento", "forno", "motor", "bomba", "painel",
        "manilha", "drenagem", "rede", "erp", "software", "estrutura",
        "tubulação", "elétrica", "hidráulica", "mecânica"
    ]

    tem_verbo_acao = any(v in texto_limpo.lower() for v in verbos_acao)
    tem_termo_tecnico = any(t in texto_limpo.lower() for t in termos_tecnicos)
    ...
    for item in decisoes_llm:
        try:
            ...
            if fase in analise_por_fase:
                ...
                # Qualidade da informação calculada pelo sistema
                ...
                if analise_por_fase[fase]["qualidade_informacao"] == "Baixa":
                    ...
                    item["alerta_qualidade"] = (
                        "Qualidade suficiente para análise."
                    )
